...
# Both Traefik API groups — containo.us is the legacy v2 group, traefik.io is v2.10+
TRAEFIK_GROUPS = ["traefik.containo.us", "traefik.io"]
...
def full_reconcile(api, tag_id):
    ...
    # Traefik IngressRoute CRDs — check both traefik.containo.us (v2) and traefik.io (v2.10+)
    for traefik_group in TRAEFIK_GROUPS:
        try:
            ingressroutes = custom.list_cluster_custom_object(
                traefik_group, "v1alpha1", "ingressroutes"
            )
            for ir in ingressroutes.get("items", []):
                ir["kind"] = "IngressRoute"
                key = build_monitor_key(ir)
                if key in seen_keys:
                    continue  # already processed from the other API group
                seen_keys.add(key)
                reconcile_resource(api, ir, managed, tag_id)
        except Exception as e:
            log.debug("IngressRoute CRD not available for %s: %s", traefik_group, e)
    ...
    log.info(
        "Reconciliation complete. Tracked %d resources (%d static, %d discovered).",
